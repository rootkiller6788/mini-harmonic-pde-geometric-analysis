...
def discreteFundamentalSolution (n : Nat) (x : Int) : Rat := randomWalkKernel n x
axiom discrete_fundamental_solution_semigroup (n m : Nat) (x : Int) : randomWalkKernel (n+m) x = finSum (2*n+1) fun y => randomWalkKernel n ((y.val:Int)-(n:Int)) * randomWalkKernel m (x-((y.val:Int)-(n:Int)))
structure ParabolicGreensFunction (n m : Nat) where G : Fin n -> Fin m -> Fin n -> Fin m -> Rat
  causality : forall x t xi tau, t.val < tau.val -> G x t xi tau = 0
def freeSpaceGreensFunction (n : Nat) (x xi : Int) (t tau : Nat) : Rat := if t < tau then 0 else discreteFundamentalSolution (t - tau) (x - xi)
axiom greens_function_representation {nx nt : Nat} (G : ParabolicGreensFunction nx nt) (u : Fin nx -> Fin nt -> Rat) (f : Fin nx -> Fin nt -> Rat) (u0 : Fin nx -> Rat) : True
def methodOfImages (G_free : Int -> Nat -> Rat) (boundary_loc : Int) (is_dirichlet : Bool) : Int -> Nat -> Rat := if is_dirichlet then fun x t => G_free x t - G_free (2*boundary_loc - x) t else fun x t => G_free x t + G_free (2*boundary_loc - x) t
def duhamelConstruction (S : SolutionOperator (Fin n -> Rat)) (u0 : Fin n -> Rat) (f : Fin m -> Fin n -> Rat) (dt : Rat) : Fin n -> Fin m -> Rat := fun i k => (S.evolve k u0) i + finSum k.val fun j_val => dt * (S.evolve (k.val-1-j_val) (fun i' => f.⟨j_val,by omega⟩ i')) i
structure ParametrixConstruction where base_kernel : Int -> Nat -> Rat
  correction : Int -> Nat -> Rat
  num_terms : Nat
axiom parametrix_convergence (K0 : Int -> Nat -> Rat) : True
