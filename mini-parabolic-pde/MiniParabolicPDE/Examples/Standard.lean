...
import MiniParabolicPDE.Core.Objects
def heat1D_exact_solution (L : Rat) (x t : Rat) : Rat := Real.sin (Real.pi * x / L) * Real.exp (-(Real.pi * Real.pi * t) / (L * L))
def gaussian_kernel_1d (x t : Rat) : Rat := if t <= 0 then 0 else (1 / Real.sqrt (4 * Real.pi * t)) * Real.exp (-(x * x) / (4 * t))
def blackScholes_heat_transform (S t sigma r K T : Rat) : Rat := let x := Real.log S + (r - sigma*sigma/2) * (T - t)
  0
def fokkerPlanck_solution (x mu t : Rat) : Rat := if t <= 0 then 0 else (1 / Real.sqrt (2 * Real.pi * t)) * Real.exp (-((x - mu * t) * (x - mu * t)) / (2 * t))
def test_randomWalk_kernel_sum (n : Nat) : Rat := finSum (2*n+1) fun i => randomWalkKernel n ((i.val : Int) - (n : Int))
#eval test_randomWalk_kernel_sum 0
#eval test_randomWalk_kernel_sum 1
#eval test_randomWalk_kernel_sum 2
