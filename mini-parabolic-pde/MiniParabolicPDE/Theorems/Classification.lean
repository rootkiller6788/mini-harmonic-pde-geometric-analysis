...
axiom classification_constant_coefficients {n : Nat} (A : Fin n -> Fin n -> Rat) (h_sym : forall i j, A i j = A j i) : True
axiom heat_equation_is_parabolic : True
structure PetrovskyParabolicity (n : Nat) where coefficients : Fin n -> Fin n -> Rat
  order : Nat
  growth_exponent : Rat
  h_growth : growth_exponent > 0
axiom heat_operator_petrovsky (n : Nat) : PetrovskyParabolicity n
structure ShilovParabolicity (n : Nat) where petrovskyPart : PetrovskyParabolicity n
  parabolicDimension : Nat
axiom shilov_parabolic_smoothing (n : Nat) (sp : ShilovParabolicity n) (u0 : Fin n -> Rat) (t : Rat) (ht_pos : t > 0) : True
structure EidelmanParabolicity (n : Nat) (m : Nat) where A : Fin m -> Fin n -> Fin n -> Rat
  parabolicityConstant : Rat
  h_constant_pos : parabolicityConstant > 0
axiom eidelman_implies_petrovsky (n m : Nat) (ep : EidelmanParabolicity n m) : PetrovskyParabolicity n
inductive QuasilinearParabolicType | nonDegenerate | degenerate | singular | mixedDegenerate deriving Repr
axiom mean_curvature_flow_parabolicity : True
